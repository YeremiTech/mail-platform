# Certificación reproducible — Mail Platform v0.10.6
...
## Preflight de adjuntos para instalaciones grandes

Las instalaciones que actualizan desde V16 o anterior y tienen muchos adjuntos heredados deben ejecutar un respaldo y después convertir los BYTEA por lotes antes de permitir que Flyway aplique V17:

```bash
export ATTACHMENT_MIGRATION_APPROVED=yes
export ATTACHMENT_MIGRATION_BATCH_SIZE=250
export ATTACHMENT_MIGRATION_PAUSE_MS=100
python3 scripts/preflight_attachment_migration.py
```

El proceso usa `FOR UPDATE SKIP LOCKED`, es idempotente y no modifica el historial Flyway. Debe finalizar con `remaining=0`. Después puede arrancarse v0.10.6 para que Flyway aplique V17. Para bases ya migradas a V17 no se ejecuta este preflight.

Antes de la conversión, comprobar espacio de disco suficiente para convivir temporalmente con BYTEA + Large Objects y conservar un respaldo verificable.

## Objetivo de capacidad

La ejecución semanal de CI utiliza 10 000 destinatarios sintéticos contra servicios descartables. El ensayo comprueba:

- admisión de correo transaccional durante el drenaje masivo;
- aceptación SMTP completa;
- captura completa en Mailpit;
- duración máxima de campaña (`LOAD_MAX_CAMPAIGN_SECONDS`);
- heap máximo observado (`LOAD_MAX_HEAP_MB`).

Los límites pueden ajustarse en un entorno dedicado, pero una relajación debe quedar documentada con la capacidad de hardware correspondiente.
